...
如果有两层相同类型的嵌套，那么就可以用 `join` 把它们压扁到一块去

这种结合的能力，functor 之间的联姻，就是 monad 之所以成为 monad 的原因。来看看它更精确的完整定义：
...
如果有一个 `Maybe(Maybe(x))`，那么 `.__value` 将会移除多余的一层，然后我们就能安心地从那开始进行 `map`。要不然，将会只有一个 `Maybe`，因为从一开始就没有任何东西被 `map` 调用。

很多情况下，总是在紧跟着 `map` 的后面调用 `join`。这时候，可以把这个行为抽象到一个叫做 `chain` 的函数里：
...
这里仅仅是把 map/join 套餐打包到一个单独的函数中。在一些库里， `chain` 也叫做 `>>=`（读作 bind）或者 `flatMap`，都是同一个概念的不同名称罢了。 `flatMap` 是最简单明了，但`chain`是 JS 里接受程度最高的一个。

* 不同类型容器调用 —— Applicative

同类型容器嵌套，使用Monad仿函数处理方式，这种方式有一个问题，那就是顺序执行问题：所有的代码都只会在前一个函数执行完毕之后才执行。其实，在很多情况下，这是非常没有必要的。其更好的替代方法就是使用applicative仿函数。applicative仿函数是实现了 `ap` 方法的pointed仿函数。其中 `pointed` 是非常重要的一个前提。`ap` 方法，类似下面这样：
...
本文主要是从思维和实践角度来学习函数式编程，针对编码中的常见问题列举了对应的基本解决思路，一些概念与其他编码方法做了类比，可能并不十分确切，请阅读时认真区分辨别。因为仅仅从思维逻辑出发，并没罗列函数式编程的过多内容和方式方法，甚至一些重要的仿函数，比如：Monad等，也没有在文中提及，参考里列举了一些文档，请自行研究阅读。

目前，社区有很多现成优秀的函数式编程库，比如：lodash/FP，ramda.js，Folktale等，文章 [Javascript的函数式库][] 还列出其他一些优秀的库，既可以在生产中使用，更可以作为学习研究的范本，值得参考。
