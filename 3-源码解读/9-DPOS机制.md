...
从现在时间点到创世时间，有一个时间段，大小假设为 t，那么 t/10 取整，就是当前时段数（getSlotNumber()）），这里的10是由 constants.interval 设定的，见文件 helpers/constants.js 21行。
...
为了安全，亿书规定受托人每轮都要变更，确保那些不稳定或者做坏事的节点被及时剔除出去。另外，尽管系统会随机找寻受托人产生新块，但是在一个轮次内，每个受托人都有机会产生一个新区块（并获得奖励），这一点与比特币的工作量证明机制（PoW）不同。这样，亿书每个区块都会与特定的受托人关系起来，其高度(height)和产生器公钥(generatorPublicKey)必是严格对应的。块高度可以轻松找到当前块的受托人周期（modules/round.js 文件51行的calc()方法），generatorPublicKey代表的就是受托人。

具体代码见 modules/round.js 文件，该文件最重要的两个方法就是 tick() 和 backwardTick() 方法。
