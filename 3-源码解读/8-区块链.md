...
任何节点，都需要先验证加载本地区块链，确保本地区块链没有被篡改，然后才可以同步其他节点区块。我们可以先设想一下，验证的时机是什么？软件启动过程为`软件启动 -> 节点联网 -> 验证区块 -> 启动完成（开始工作）`，这里，当联网成功之后，就应该立刻检查区块链是否完整，不然下一步工作没法开始。
...
408行： 将调用logic/account.js的createTables()方法，创建与用户相关的帐号信息表，全部以“mem_”开头，主要包括“mem_accounts”，“mem_accounts2contacts”，“mem_accounts2u_contacts”，“mem_accounts2delegates”，“mem_accounts2u_delegates”，“mem_accounts2multisignatures”，“mem_accounts2u_multisignatures”，“mem_round”等7个表。这些信息与独立用户相关，不需要被其他节点同步，因此在同步时创建。这与本部分第一篇里列出的表格不同，那些表格是需要同步处理的。

如果是新客户端，数据库为初始创建，创世区块第一次写入，count == 1，会立刻调用闭包load()函数（428行），开始同步缺失的区块。我们先不考虑其他情况，直接看357行的load()函数，可以发现该函数先删除帐号表格（removeTables），然后重建（createTables），并通过“async.until”方法（369行）进行循环加载区块链数据，具体方法是 modules/blocks.js的loadBlocksOffset()。当 count < offset 返回 “true” 的时候，循环结束，区块链数据同步完毕，自然激活 “blockchainReady” 事件（398行）。

这里的 “Offset” 不是分页数据，而是一次同步的区块数量，这样做的好处是避免一次性同步全部区块链，导致数据请求量过大，影响计算机性能。这个值最大是 limit 的值（380行），limit 等于 “config.json” 文件设定的全局变量 loading.loadPerIteration。用户可以修改该配置文件，在启动软件时选择自己的配置文件，从而实现定制。如下：
...
上面说到 modules/blocks.js 的 loadBlocksOffset() 方法才是处理同步的核心方法，
