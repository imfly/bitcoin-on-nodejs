...
另一个类似的地址，就是区块链的以c结尾的块地址，用来标注`generatorId`，代码与上密的基本一致：
...
分析代码949行和984行，可以了解到，所谓的`注册用户名`，实质上就是提交了一个`TransactionTypes.USERNAME`类型的交易。另外，因为亿书提供了多重签名和支付密码功能，对每笔交易都要进行验证，这部分我们在后面分析。关于交易如何验证和创建，您也可以查看交易部分的流程图。
