...
分析一下，与传统电子商务网站的交易相比，交易本身就是从一个人（我们上面分析了地址在系统里其实就扮演了人这个角色）到另一个人的价值转移，没什么特别的。后面进一步的解释，所谓价值转移的数据结构也不是什么新鲜东西，不过是多了一个签名运算而已。最大的不同在于，要经过比特币网络（P2P网络）传输，由节点保存到区块，这才是真正复杂的过程。

把这些重要信息汇总到一起，既让用户容易理解，又要体现加密货币特点，可以这样定义：

> 加密货币的交易是指人们通过加密货币网络，把加密货币从一个地址转移到另一个地址，
> 并把相关信息数据由网络节点保存到区块链的过程。

交易是加密货币系统中最重要的部分，系统中任何其他的部分都是为了确保交易可以被生成、能在网络中得以传播和通过验证，并最终添加入全球交易总账簿（区块链）。因此，从开发设计角度考虑，完成一笔加密货币交易必须实现下面的基本需求：
...
除了第1条，其他三条，只要缺少一个，就不是去中心化的交易实现。下面，我们来详细阅读分析亿书的交易是如何实现的。
...
交易是人类行为，必然涉及到甲乙双方（货币发送者和接收者，我们用甲乙方来代替，下文同）和交易数额。甲方必须是主动发起交易的有效用户（有合法亿书币客户端地址，需要验证），是亿书币的支付方，是交易的支付来源。乙方比较灵活，可以是另一个有合法客户端地址的用户（需要验证），也可以是亿书系统本身（功能性交易），是亿书币的接收方（需要通过类型区分对待）。
