...
交易，在汉语词典里，既可以是名词，也就是代表交易内容的数据信息（技术上就是数据结构），又可以是动词，代表一个操作过程。把这些重要信息汇总到一起，既让用户容易理解，又要体现加密货币特点，可以这样定义一个交易操作：

> 加密货币的交易是指人们通过加密货币网络，把加密货币进行有效转移，
> 并把交易数据由点对点网络保存到区块链的过程。

即便简化定义，对于刚刚接触加密货币的小伙伴来说，理解起来，还是有点费解。通常，大家喜欢把加密货币的交易，比做纸质支票。与支票相似，一笔加密货币交易其实是一个有着货币转移目的的工具，这个工具只有在交易被执行时才会在金融体系中体现，而且交易发起人并不一定是签署该笔交易的人。支票本身就是记录一笔交易的数据结构，从签署支票到兑付完成的过程就是一个交易操作行为。
...
因此，本质上，加密货币交易是价值所有权的转移，而不是价值转移。加密货币总量就是那些，从始至终都不会变化，人为丢失的是人类使用流通的私钥权限，总量仍在网络上不会丢失。记录加密货币总量的区块链就那一条，这个链条可以越来越长，越来越大，但是增长的仅仅是交易的信息，即价值所有权变更信息。用个不慎确切的比喻，加密货币就像一列永不停息的火车，上下的是人次，固定的是座位，您只有在自己的人生旅途中才拥有某个座位的所有权（使用权）。

从设计原理上说，加密货币淡化了交易者帐号，简化为输入输出，所谓的账户也只是存在于客户端钱包这类具体的应用层的软件里，就像那列火车总要有火车站吧，而某一段旅程的火车票是有所属的，是要与真实的帐号或身份对应的，所以角色存在于客户端钱包等各类应用软件里。

亿书的原理也是如此，只不过亿书通过进一步扩展交易类型，强化了用户帐号的存在，使得更加适合处理各类资产所有权，从而为数字版权保护奠定良好架构基础。

#### 交易生命周期

交易是加密货币系统中最重要的部分，系统中任何其他的部分都是为了确保交易可以被生成、能在网络中得以传播和通过验证，并最终写入全球交易总账簿——区块链。因此，从开发设计角度考虑，一笔加密货币交易必须包括下列过程：

1. 生成一笔交易。这里是指一条包含交易双方加密货币地址、数量、时间戳和有效签名等信息的合法交易数据;
2. 广播到网络。几乎每个节点都会获得这笔交易数据。
3. 验证交易合法性。生成交易的节点和其他节点的都要验证，没有提供验证的交易，是不能进入加密货币网络的。
4. 写入区块链。
...
亿书已经完成或正在开发的交易类型，目前包括14种（后续会有更多），分别是：
...
亿书交易类型尽管多样，但是交易的基本逻辑是一样的。加密货币的整体设计就是为了安全交易，整个加密货币都是交易逻辑的有效组成部分，要比传统电子商务网站复杂的多，但与交易直接相关的代码，却又非常简单清晰。结合加密货币交易实现基本需求，完整实现一笔交易，亿书需要这样几个步骤：

（1）生成交易数据

交易是人类行为，涉及到甲乙双方（货币发送者和接收者，我们用甲乙方来代替，下文同）和交易数额，这在很多交易，特别是版权交易方面更加重要。甲方是主动发起交易的有效用户，是亿书币的支付方，是交易的支付来源。乙方比较灵活，可以是另一个有合法客户端地址的用户，也可以是亿书系统本身（功能性交易），是亿书币的接收方。
...
整理数据的另一个重要过程，就是检验这些交易数据的正确性。这个很好理解，如果一个人胡乱填写接受地址，也能把币发送出去，那就笑话了。但过程较为繁琐，主要涉及到：发起交易的用户是否存在、密钥是否正确、是否多重签名帐号、是否有支付密码，以及接受方用户地址是否合法等，都要逐个检验。
...
一笔合法交易，使用甲乙方的公钥签名，确保交易所属。同时，还要要准确记录它的交易时间戳，方便追溯。交易ID包含了丰富加密信息的字段，需要复杂的生成过程。
...
这里的交易合法性，除了基本信息正确之外，主要是指确认交易是未确认的交易，也不是用户重复提交的交易，即双花交易。所谓双花交易，通俗的说，就是是用户在交易确认之前，又一次提交了相同交易信息，导致一笔钱花两次，这种情况是必须要避免的。

不是先广播到网络，然后让其他节点验证吗？当然是的，但是想象一下，一个看似合法但是无法通过验证的交易，该不该广播到网络上呢？显然不能。所以在广播到网络之前必须验证交易合法性，不合法的交易没有机会广播到网络。节点收到新的交易信息时，要重新验证。如此一来，任何对网络的攻击，都只会影响一个节点，安全性大大提高。

验证合法的交易就可以直接加入区块链了，因此从上面的第一步到现在，都是在一个节点上完成的。这也为下面的广播处理打下基础，一旦交易被广播到网络，在其他节点，这里的验证和处理过程就会重复执行一次。
...
这里的编码逻辑非常清晰，但是具体开发起来，却是非常核心的部分，需要比较深厚的开发功底。大量的回调和验证，也给这样复杂的编程带来很多困扰。因此，好好熟悉基本编码技巧，从小处着手打好基础很重要。

本文涉及的流程图非常复杂，我也花费了大量时间去处理，但是很多细节仍然无照顾到，也无法保证有错误疏漏的地方，请看到问题的小伙伴技术反馈给我。

交易是怎么写入区块链的，上面仅仅提到了，但是并不够详细和深入，因为简单的论述并不能解释清楚区块链的原理，还是专门拿出一篇来，详细讲述比较好。而且，作为目前加密货币的“网红”，区块链也值得我们好好研究研究。请看下一篇：**《神秘的区块链》**
