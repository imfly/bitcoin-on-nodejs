...
从设计原理上说，加密货币本身是不存在账户的，所谓的账户也只是存在于客户端钱包这类具体的应用层的软件里，就像那列火车总要有火车站吧，而某一段旅程的火车票是有所属的，是要与真实的帐号对应的，所以角色存在于客户端钱包等各类应用软件里。

亿书的原理也是如此，只不过亿书通过进一步扩展交易类型，使得更加适合处理各类资产所有权，从而为数字版权保护奠定良好架构基础。
...
加密货币淡化了交易者帐号，简化为输入输出，亿书通过扩展交易类型，强化了用户帐号的存在。交易是人类行为，涉及到甲乙双方（货币发送者和接收者，我们用甲乙方来代替，下文同）和交易数额，这在很多交易，特别是版权流转方面更加重要。甲方是主动发起交易的有效用户，是亿书币的支付方，是交易的支付来源。乙方比较灵活，可以是另一个有合法客户端地址的用户，也可以是亿书系统本身（功能性交易），是亿书币的接收方。
...
不是先广播到网络，然后让其他节点验证吗？当然是的，但是想象一下，一个看似合法但是无法通过验证的交易，该不该广播到网络上呢？显然不能。所以在广播到网络之前必须验证交易合法性，不合法的交易没有机会广播到网络。节点收到新的交易信息时，重新验证。如此一来，任何对网络的攻击，都只会影响一个节点，安全性大大提高。
