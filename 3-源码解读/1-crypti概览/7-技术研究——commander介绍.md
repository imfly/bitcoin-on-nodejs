...
我相信，以后介绍的很多组件或模块，网上都会有很多相关文档。我的做法自然也规避了文字雷同的可能，毕竟人的思维可以相似，但不会相同。

下面，以commander、domain和async在app.js中出现的顺序为准，分别介绍。

## Commander

其实，在Nodejs或ruby等语言环境里，将代码转为命令行执行，只要在文件头部添加一行所谓的`shebang`就可以执行了。难在选项设置和流程控制，所以才有了这类工具的出现，叫它们`命令行框架`最好。
...
原本把三个组件（模块）一起整理发布的，发现写到这已经很长了。原想只不过简单介绍，但写出来，居然这么罗嗦，文笔不行，大家还是忍受着吧，待我慢慢提高。

这篇仅仅介绍了`commander`组件的相关概念和使用，关于如何安装（简单一条命令）、如何修改权限、如何打包发布，都没有细说。这类知识，我坚信网上一搜多的是，不过，还是建议最好去浏览官网文档。
