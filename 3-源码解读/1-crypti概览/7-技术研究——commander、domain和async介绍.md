...
我坚信，搞编程的都是学习高手，智商很高。但不同的人，方法迥异。有的人天生聪颖，翻翻源码和文档，就能在日后自由使用。相反，有的人就不行，我个人就算典型的一个，坚信自己愚钝至极。

我经常遇到这样的情况，对于某个组件，这次用了，下次有需要，还要再看文档，特别是对于方法的选择上，有时候需要反复试验。所以，多年来，我认真总结和改进自己的做法，努力向高手靠拢。
...
事实上，概念中含有逻辑，是逻辑的高级浓缩版。概念清晰，逻辑必然是清晰的，相反却未必。因此，学习知识，掌握概念最重要；创新知识，创造概念最重要。
...
这个`名字`，是工具的名字（其实也是`命令`，我叫它主命令），用来区分系统命令，限定`命令`使用上下文。我通常用工程的名字或操作对象的名字代替，是个名词，比如：`book`。而用`Commander`写的`命令`是个动词，比如：`generate`，结果形式如下：
...
我们想到的通常是`业务逻辑`，不过，请注意，只要是开发，逻辑部分自然只能开发者自己实现，所以，`Commander`仅仅提供了一个接口函数而已。这里的`命令`，仅是一个名词。`帮助`是提示，也仅是简单的文本信息。剩下的各种`选项`，可以规范，也最为关键，才是`Commander`的可爱之处。

a.**命令**： 使用`command`函数定义（子命令），例如
...
小结：中规中矩地定义每一个命令`command`方法没有描述信息，跟着`action`方法。如果定义类似git类型的，一连串的命令，一个一个来，显然麻烦，就把描述信息放在`command`里，不跟`action`方法，这时默认请求对应的js文件，如：
...
b.**选项**：使用`option`定义，可以理解为`命令行数据结构`。

该函数很简单，可以方便开发者将文本输入转化为程序需要的数据结构。注意其功能点：

* 可以设置任何数量的选项，每一个对应一个`.option`函数调用;
* 可以设置默认值;
* 通过处理函数，可以提供文本、数值、数组、集合和范围等约束类型;
* 可以使用正则表达式;

小结：简单使用就是选项名称和描述;复杂一点就要提供处理函数;再复杂就用`arguments`方法代替`option`，使用`可变参数`。

c.**帮助**： 使用`help`函数输出一切有用的描述信息，这些信息通常在命令和选项的定义中，例如
...
d.**逻辑**： 使用`action`方法注册逻辑，将代码转向执行自己的逻辑代码。例如
